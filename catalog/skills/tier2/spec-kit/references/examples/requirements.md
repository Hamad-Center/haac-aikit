...
Tone is carried through **placeholder text and labels only** — the form copy uses in-world language. The form structure and validation are neutral/functional. Example labels:

- Name label: "Agent ID" — placeholder: "e.g. GPT-4-turbo, Claude, Gemini Pro"
- Email label: "Contact Address" — placeholder: "optional — for follow-up by our staff"
- Message label: "Describe Your Experience"
- Rating label: "Session Rating" — helper: "1 = Still suffering, 5 = Achieved equilibrium"

## Out of Scope

- No client-side JS — form submits via standard HTML POST
- No auth or admin moderation
- No email notifications on submission

## Decisions

### No client-side JS
Matches the existing stack (Hono JSX SSR, no JS bundle). Form submits via standard HTML POST.

### No ORM
Raw SQL via `better-sqlite3`, consistent with existing `src/db/` patterns.

### Email is optional
Stored as nullable in the schema; no email validation beyond `type="email"` on the input.

### POST/redirect/GET
After a successful submission, redirect back to `/feedback` to prevent double-submit on reload.

## Context

- See `specs/mission.md` — the codebase is a teaching example, keep the implementation idiomatic and easy to follow.
- See `specs/tech-stack.md` — Hono 4.x + Hono JSX (SSR) + SQLite + Vitest. No new dependencies should be needed.
