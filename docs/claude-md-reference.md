...
> the canonical 2026 sources listed at the bottom. Used to design the rules setup for
> this repo (`/AGENTS.md`, `/CLAUDE.md`, `.claude/rules/`, `.claude/settings.json`).
...
This is exactly what `catalog/rules/CLAUDE.md.shim` ships and what this repo uses.
...
# Marker engine rules
- Never edit content outside the BEGIN/END markers...
```
...
### Compaction
- **Project-root CLAUDE.md survives `/compact`** — it is re-injected from disk.
...
## 10. Conclusions for this repo (haac-aikit)

What we applied based on this reference:

1. **`/AGENTS.md`** — canonical, 77 lines, structured per `catalog/rules/AGENTS.md.tmpl`.
   Uses `IMPORTANT:` emphasis on the highest-stakes gotchas. Includes a compaction-preservation hint.
2. **`/CLAUDE.md`** — 5-line shim (`@AGENTS.md` + empty BEGIN/END override block).
3. **`/.claude/rules/markers.md`** — path-scoped rule (paths: `src/render/**`,
   `catalog/**.tmpl`). Demonstrates the load-on-demand feature and protects the
   marker-engine invariant in the place where it matters.
4. **`/.claude/settings.json`** — dogfoods `catalog/settings/settings.json`:
   permissions allowlist for npm/git, deny list for secrets, ask list for
   destructive git ops.
5. **`.gitignore`** — adds `CLAUDE.local.md`, `.claude/settings.local.json`,
   `.claude/.credentials.json` per Anthropic recommendation.

Skipped intentionally:
- **Auto memory configuration** — leave defaults. Per-developer concern; not
  appropriate to pin in a shared repo.
- **Managed-policy CLAUDE.md** — only relevant for org-wide deployments.
- **`claudeMdExcludes`** — repo isn't in a monorepo with parent CLAUDE.md noise.
...
_Last refreshed: 2026-04-29 against the redirected `code.claude.com/docs/en/*` namespace (formerly `docs.claude.com/en/docs/claude-code/*`)._
